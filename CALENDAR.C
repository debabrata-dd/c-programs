	/* This is a program which asks for month and year,
	   generates the calender of the particular month of that
	   particular year.
...
void PrintFile(int M,int Y, int Z)
{
	...
	for(i=1;i<=MAX_NO;i++)
		fprintf(stream,"-");
	fprintf(stream,"\n\n\t\tCreated by: Debabrata Das [98316-59440]");
	fclose(stream);
}
...
Top:    /* goto statement */
	textcolor(WHITE);
	clrscr();
	printf("\n\tThis program shows the corresponding calender of \n");
	...
	do
	{
		...
		textcolor(11);
		textbackground(9);
		cprintf("Created by: Debabrata Das [98316-59440]");
		textcolor(WHITE);
		textbackground(BLACK);
